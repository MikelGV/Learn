...
// streamline our error check below.
...
func main() {
    ...
    // The most basic file reading task is slurping a file's entire contents into
    // memory.
    ...
    // For example, reads like the ones above can be more robustly implemented
    // with ReadAtLeast.
    o3, err := f.Seek(6, 0)
    check(err)
    b3 := make([]byte, 2)
    ...
    // Bufio package implements a buffered reader that may be useful both for its
    // efficency with many small reads and because of the additional reading methods
    // it provides.
